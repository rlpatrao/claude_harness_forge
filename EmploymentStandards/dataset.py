...
class SalaryDataset:
    ...
    def suggest_alternatives(self, current_filters: dict, n: int = 3) -> list[dict]:
        ...
        for spread, dim, grp in spread_rank[:2]:
            top_vals = grp.sort_values("cnt", ascending=False).head(4).index.tolist()
    ...
    def _balance(self, df: pd.DataFrame) -> dict:
        ...
        for col, label in check.items():
            ...
            if len(vc) == 1:
                warnings.append(f"Single-source: all {n} rows share {label} '{vc.index[0]}'.")
    ...
    def _stats(self, df: pd.DataFrame) -> dict:
        if len(df) < self.MIN_ROWS_HARD:
            return {"error": f"Only {len(df)} row(s) match — guardrail blocks output below {self.MIN_ROWS_HARD} rows."}
        s = df["salary_usd"]
        out: dict[str, Any] = {
            "count": len(df),
            "salary_usd": {
        ...
        for col in ["experience_level", "company_location", "employment_type", "industry", "company_size", "remote_ratio"]:
    ...
    def _build_options(self) -> dict[str, list]:
        out: dict[str, list] = {}
        for col in ["experience_level", "employment_type", "company_size", "company_location",
                    "employee_residence", "industry", "education_required", "job_category", "hiring_status"]:
