...
def _generate(n: int = 15001, seed: int = 42) -> pd.DataFrame:
    ...
    loc_mult  = {
        "US": 1.00, "GB": 0.85, "DE": 0.78, "CA": 0.80, "AU": 0.72,
        "IN": 0.30, "FR": 0.75, "NL": 0.82, "SG": 0.88, "CH": 1.15,
        "BR": 0.28, "JP": 0.65, "ES": 0.60, "PL": 0.40, "SW": 0.80,
    }
    loc_p = np.array([r[3] for r in LOCS]); loc_p /= loc_p.sum()

    INDUSTRIES = [
        "Technology", "Finance", "Healthcare", "E-commerce", "Automotive",
    ...
    sal_range  = {"EN": (55000, 85000), "MI": (85000, 135000),
                  "SE": (130000, 200000), "EX": (175000, 320000)}
    ...
    yrs_exp = np.array([
        int(rng.integers(yrs_range[e][0], yrs_range[e][1]))
        for e in exp_arr
    ])
    skills_list = [
        ", ".join(rng.choice(SKILLS, int(rng.integers(3, 7)), replace=False))
        for _ in range(n)
    ]
    residence = [
        rng.choice(loc_codes, p=loc_p) if rng.random() < 0.10 else loc_arr[i]
        for i in range(n)
    ]
    base_date = pd.Timestamp("2022-01-01")
    dates = [
        (base_date + pd.to_timedelta(int(d), "D")).strftime("%Y-%m-%d")
        for d in rng.integers(0, 1200, n)
    ]
    ...
    return pd.DataFrame({
        ...
        "employment_type":    emp_arr,
        "company_size":       size_arr,
        "company_location":   loc_arr,
        "employee_residence": residence,
        "remote_ratio":       remote_arr,
        "education_required": edu_arr,
